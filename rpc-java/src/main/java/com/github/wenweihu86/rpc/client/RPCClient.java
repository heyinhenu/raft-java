...
public class RPCClient {
    ...
    public <T> Future<T> sendRequest(final Long callId, Object fullRequest, Class<T> responseClass,
        ...
        RPCFuture future = new RPCFuture(scheduledFuture, callId, fullRequest, responseClass, callback,
                channelInfo.getChannelGroup());
    ...
    private void init(RPCClientOptions options) {
        ...
        bootstrap.group(
                new NioEventLoopGroup(options.getIoThreadNum(), new CustomThreadFactory("client-io-thread"))).handler(
                initializer);
    }
    ...
    private void addEndPoint(EndPoint endPoint) {
        ...
        allConnections.add(
                new RPCChannelGroup(endPoint.getIp(), endPoint.getPort(), rpcClientOptions.getMaxConnectionNumPerHost(),
                        bootstrap));
        endPoints.add(endPoint);
    }
